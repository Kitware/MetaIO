...
/** Stolen from kwsys */
static std::string
GetCurrentDateTime(const char * format)
{
  char   buf[1024];
  time_t t;
  time(&t);
...
/****/
MetaOutputStream::MetaOutputStream()
{
  m_Enable = true;
  m_IsStdStream = false;
  m_Name = "";
...
void
MetaOutputStream::SetName(const char * name)
{
  m_Name = name;
}

void
MetaOutputStream::Enable()
{
  m_Enable = true;
}

void
MetaOutputStream::Disable()
{
  m_Enable = false;
}

void
MetaOutputStream::SetStdStream(std::ostream * stream)
{
  m_StdStream = stream;
  m_IsStdStream = true;
}
...
std::ostream *
MetaOutputStream::GetStdStream()
{
  return m_StdStream;
}

std::string
MetaOutputStream::GetName() const
{
  return m_Name;
}

bool
MetaOutputStream::IsEnable() const
{
  return m_Enable;
}

bool
MetaOutputStream::Write(const char * buffer)
{
  if (m_IsStdStream)
  {
    *m_StdStream << buffer;
...
void
MetaOutputStream::SetMetaOutput(void * metaOutput)
{
  m_MetaOutput = metaOutput;
}

/****/
MetaFileOutputStream::MetaFileOutputStream(const char * name)
  : MetaOutputStream()
{
  m_FileName = name;
...
std::string
MetaFileOutputStream::GetFileName()
{
  return m_FileName;
}
...
/** Add a field */
bool
MetaOutput::AddField(std::string  name,
...
/** Add a float field */
bool
MetaOutput::AddFloatField(std::string name,
                          std::string description,
                          float       value,
                          std::string rangeMin,
                          std::string rangeMax)
...
/** Add a int field */
bool
MetaOutput::AddIntField(std::string name,
                        std::string description,
                        int         value,
                        std::string rangeMin,
                        std::string rangeMax)
...
/** Add list field */
bool
MetaOutput::AddListField(std::string name, std::string description, ListType list)
{
  Field field;
  field.name = name;
  field.description = description;
...
/** Add meta command */
void
MetaOutput::SetMetaCommand(MetaCommand * metaCommand)
{
  ...
  m_MetaCommand->SetOption("GenerateXMLMetaOutput", "", false, "Generate XML MetaOutput to the console");
  m_MetaCommand->SetOptionLongTag("GenerateXMLMetaOutput", "oxml");
  m_MetaCommand->SetOption(
    "GenerateXMLFile", "", false, "Generate XML MetaOutput to a file", MetaCommand::STRING, "", MetaCommand::DATA_OUT);
  m_MetaCommand->SetOptionLongTag("GenerateXMLFile", "ofxml");
}

/** Get the username */
std::string
MetaOutput::GetUsername()
{
#if defined(_WIN32) && !defined(__CYGWIN__)
  char  buf[1024];
...
std::string
MetaOutput::TypeToString(TypeEnumType type)
{
  switch (type)
  {
    case INT:
...
std::string
MetaOutput::GenerateXML(const char * filename)
{
  std::string buffer;
  ...
  buffer += "<Creation date=\"" + GetCurrentDateTime("%Y%m%d") + "\"";
  buffer += " time=\"" + GetCurrentDateTime("%H%M%S") + "\"";
...
MetaOutput::Write()
{
  ...
  {
    // this->GenerateXML();
    std::string   filename = m_MetaCommand->GetValueAsString("GenerateXMLFile");
    std::ofstream fileStream;

#ifdef __sgi
    fileStream.open(filename.c_str(), std::ios::out);
#else
    fileStream.open(filename.c_str(), std::ios::binary | std::ios::out);
#endif
  ...
  while (itStream != m_StreamVector.end())
  {
    ...
    if (!(*itStream)->Open())
    {
      std::cout << "MetaOutput ERROR: cannot open stream" << std::endl;
      return;
    }
    ...
    while (it != itEnd)
    {
      ...
      {
        std::string filename = ((MetaFileOutputStream *)(*itStream))->GetFileName();
    ...
    if (!(*itStream)->Close())
    {
      std::cout << "MetaOutput ERROR: cannot close stream" << std::endl;
      return;
    }
    ++itStream;
  }
...
void
MetaOutput::AddStream(const char * name, MetaOutputStream * stream)
{
  stream->SetName(name);
  m_StreamVector.push_back(stream);
}
